...
export interface Menu {
  id: string;       // TEXT（主キー）
  name: string;     // TEXT（メニュー名）
  price: number;    // INTEGER（価格）
  category: string; // TEXT（カテゴリ）
}
 ...
 * @returns 注文オブジェクトの配列 (KitchenOrder[])
 */
...
export async function updateOrderStatus(
    ...
    // PUT /api/orders/:id/status に合わせる
    ...
    try {
        ...
        if (!response.ok) {
            // サーバーからのエラー（4xx, 5xx）を処理
            throw new Error(
                `API Error (${response.status}): ${data.error || 'Unknown error'}`
            );
        }

        // 成功時のレスポンス (message, id, statusを含む) を返す
        return data;
...
/**
 * 特定テーブルの注文を取得
 * @param tableNumber 取得したいテーブル番号 (例: "5" → T-5)
 */
export async function fetchOrdersByTable(tableNumber: string): Promise<Order[]> {
  ...
  try {
    ...
    return apiOrders.map(o => ({
      ...
      items: o.items || [], // DBにitemsがある場合はそのまま
...
/**
 * メニューを削除（DELETE /api/admin/menu/:id）
 */
