...
export async function fetchKitchenOrders(): Promise<KitchenOrder[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch kitchen orders: ${response.status} - ${
...
export async function fetchOrdersByTable(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `注文取得失敗: ${res.status} - ${errorData.error || "Unknown error"}`
      );
    }
...
// ✅ 指定されたテーブル番号の注文を取得
export async function fetchOrderTableNumber(
  tableNumber: string
): Promise<Order[]> {
  const url = `${API_BASE_URL}/api/orders?tableNumber=${tableNumber}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `注文取得失敗: ${res.status} - ${err.error || "Unknown error"}`
    );
  }
